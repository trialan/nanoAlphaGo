...
policy_optimizer = optim.Adam(policyNN.parameters(), lr=learning_rate_policy)
value_optimizer = optim.Adam(valueNN.parameters(), lr=learning_rate_value)


def ppo_train(policyNN, valueNN, board):
    for k in range(N_trajectories):
        trajectories = collect_trajectories(policyNN, board)
        rewards_to_go = compute_reward_to_go(trajectories)
        advantages = compute_advantages(rewards_to_go, valueNN, trajectories)

        for _ in range(policy_epochs):
            update_policy_ppoclip(policy_optimizer, policyNN, trajectories, advantages, epsilon)
        for _ in range(value_epochs):
            update_value_function_mse(value_optimizer, valueNN, trajectories, rewards_to_go)


def collect_trajectories(policyNN, board):
    pass
